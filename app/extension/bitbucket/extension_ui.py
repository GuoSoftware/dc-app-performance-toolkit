...
from selenium_ui.bitbucket.pages.pages import PopupManager
...
def app_specific_action(webdriver, datasets):
    ...
    def measure():
        ...
        def sub_measure():            
            ...
            page.wait_until_visible((By.ID, 'prSquashCommitMessage')).send_keys(page.generate_random_string(5))

            # select commit author
            dropdown = page.select(page.get_element((By.ID, 'prSquashCommitAuthor')))
            desired_option = None
            for option in dropdown.options:
                if "perf-user" in option.text:
                    desired_option = option
                    break
            if desired_option:
                dropdown.select_by_visible_text(desired_option.text)

            # submit the request
            page.wait_until_clickable((By.ID, 'prSquashSubmit')).click()
            try:
                page.wait_until_invisible((By.ID, 'prSquashSubmit'), timeout=40) # Expect the dialog with prSquashButton button to disppear after the PR page reload after the success
